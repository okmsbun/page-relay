...
function harness(chrome = {}, fixtureProviders = []) {
  ...
  vm.runInContext(
    fs.readFileSync(path.join(__dirname, "../ai-providers.js"), "utf8"),
    context,
  );
  vm.runInContext(
    fs.readFileSync(path.join(__dirname, "../deepseek-discovery.js"), "utf8"),
    context,
  );
  ...
  for (const id of fixtureProviders)
    vm.runInContext(
      `AIProviders.get(${JSON.stringify(id)}).sending = true`,
      context,
    );
  vm.runInContext(
    fs.readFileSync(path.join(__dirname, "../chat-destinations.js"), "utf8"),
    context,
  );
...
test("permitted incognito is explicitly labelled and duplicate chat tabs are consolidated", () => {
  ...
  const entries = api.describeTabs(
    [
      tab(1),
      tab(2, 20, { url: tab(1).url }),
      tab(3, 30, { incognito: true }),
      tab(4, 10, { url: "https://chatgpt.com/" }),
      tab(5, 20, { url: "https://chatgpt.com/" }),
    ],
    10,
    true,
  );
...
test("only conversation/new-chat URLs accepted", () => {
  ...
  for (const url of [
    ...
    "https://chatgpt.com/g/g-custom/c/abc",
...
test("ambiguous submission is marked for review, not reported sent", async () => {
  ...
  const results = await api.sendSelected(
    ...
    {},
    async () => {
      const error = new Error("Tab closed after submission");
...
test("pre-send busy is temporary, other chats continue, and retry is explicit", async () => {
  ...
  const api = harness({
    tabs: { get: async (id) => tabs.find((item) => item.id === id) },
  });
  const choices = api.describeTabs(tabs, 10, false);
  const calls = [];
  const results = await api.sendSelected(
    ...
    async (destination) => {
      calls.push(destination.id);
      if (destination.id === 1)
        throw Object.assign(new Error("Generating a response"), {
          busy: true,
          needsReview: false,
        });
  ...
  assert.deepEqual(
    ...
    ["busy", "sent", "failed", "review"],
  );
  assert.deepEqual(calls, [1, 2, 3, 4]); // No automatic retry.
  const retried = await api.sendSelected(
    [choices[0]],
    {},
    async (destination) => {
      calls.push(destination.id);
...
test("only five providers remain, and unimplemented senders stay discovery-only", () => {
  ...
  const urls = [
    ...
    "https://gemini.google.com/app/three",
    "https://chat.deepseek.com/a/chat/s/four",
    "https://copilot.microsoft.com/chats/five",
  ];
  const entries = api.describeTabs(
    urls.map((url, i) => tab(i, 10 + i, { url })),
    10,
    false,
  );
  assert.deepEqual(
    Array.from(entries, (item) => item.providerId),
  ...
  assert.ok(
    entries.slice(3).every((item) => item.discoveryOnly && item.unavailable),
  );
});
...
test("provider login, share, marketing pages and spoofed domains are excluded", () => {
  ...
  for (const url of [
    ...
    "https://claude.ai/share/one",
    "https://gemini.google.com/share/two",
    "https://grok.com/share/three",
    "https://www.perplexity.ai/hub",
    "https://copilot.microsoft.com/settings",
    "https://claude.ai.evil.test/chat/a",
    "https://claude.ai:123/chat/a",
    "https://someone@claude.ai/chat/a",
  ]) {
...
test("discovery-only destinations cannot reach an adapter; later selected chats still send", async () => {
  const tabs = [
    tab(1, 10, { url: "https://chat.deepseek.com/a/chat/s/test" }),
    tab(2),
  ];
  const api = harness({
    tabs: { get: async (id) => tabs.find((item) => item.id === id) },
  ...
  const calls = [];
  const results = await api.sendSelected(
    api.describeTabs(tabs, 10, false),
    {},
    async (destination) => {
      calls.push(destination.id);
      return { sent: true };
...
test("DeepSeek roots need a verified composer; existing conversations keep URL discovery", async () => {
  ...
  const api = harness({
    tabs: {
      query: async () => [
        tab(1, 10, { url: "https://www.deepseek.com/en/" }),
        tab(2, 10, { url: "https://chat.deepseek.com/" }),
        tab(3, 20, { url: "https://chat.deepseek.com/a/chat/s/abc" }),
      ],
    },
    scripting: {
      executeScript: async (options) => {
        probes.push(options);
        return [{ result: options.target.tabId === 2 }];
      },
    },
    windows: { getCurrent: async () => ({ id: 10 }) },
    extension: { isAllowedIncognitoAccess: async () => false },
  ...
  assert.deepEqual(
    probes.map((options) => options.target.tabId),
    [1, 2],
  );
  assert.ok(
    probes.every(
      (options) =>
        options.target.frameIds[0] === 0 &&
        options.func.name === "hasDeepSeekComposer",
    ),
  );
});
...
test("removed providers, read-only shares, account pages and spoofed hosts are excluded", () => {
  const { identity } = harness();
  for (const url of [
    "https://www.meta.ai/share/a",
    "https://www.meta.ai/login",
    "https://poe.com/login?redirect_url=/",
    "https://poe.com/s/shared",
    "https://poe.com/settings",
    "https://poe.com.evil.test/chat/x",
...
test("manifest grants the source page and every provider host, and preserves MV3 permissions", () => {
  ...
  assert.ok(granted.includes("<all_urls>"));
  for (const pattern of patterns) {
    const origin = new URL(pattern.replace(/\*$/, "")).origin;
    assert.ok(
      granted.includes(pattern) || granted.includes("<all_urls>"),
      `${origin} must be reachable for delivery`,
    );
  }
...
test("favicons accept only HTTPS provider-host assets and otherwise use a local fallback", () => {
  ...
  const entries = api.describeTabs(
    urls.map((favIconUrl, i) => tab(i, 10, { favIconUrl })),
    10,
    false,
  );
...
test("cross-provider queue isolates failures and uncertainty without retrying or changing payload", async () => {
  const tabs = [
    tab(1),
    tab(2, 20, { url: "https://claude.ai/chat/two" }),
    tab(3, 30, { url: "https://gemini.google.com/app/three" }),
    tab(4),
  ];
  const api = harness(
    { tabs: { get: async (id) => tabs.find((item) => item.id === id) } },
  ...
  const results = await api.sendSelected(
    ...
    async (destination, value) => {
      ...
      if (destination.providerId === "claude")
        throw new Error("Existing draft");
      if (destination.providerId === "gemini") {
  ...
  assert.deepEqual(
    Array.from(results, (item) => item.state),
    ["sent", "sent", "failed", "review"],
  );
});
